...
| `marca/VOZ.md` | **la voz de la agrupación** (24/9): cómo se escribe lo que sale en nombre de la app, qué no se promete y cómo se etiquetan los links. Se le pasa a la IA antes de pedirle un texto (el plugin *Marketing* de Claude, NotebookLM, Gemini). Junta reglas que ya estaban decididas; si choca con `LEEME.md`, manda el LEEME |
...
| 24/9 | **`marca/VOZ.md`: la guía de voz para escribir placas, epígrafes y textos de la app.** Junta lo que estaba repartido en `LEEME.md`, `BITACORA.md`, `PROMPT-DISENO.md` y los comentarios de las pantallas: quién habla (nosotres, de vos, con la e), los mensajes de fondo, cómo se llaman las secciones, lo que no se promete, «Qué dice la placa» del panel y las etiquetas `?de=`. **Lo que no estaba escrito no se inventó**: hashtags, firma, temas a evitar y placas de ejemplo quedan como lista para que complete la agrupación. Sin `v` nueva: no toca la app. `marca/` está fuera del grafo | `—` |
