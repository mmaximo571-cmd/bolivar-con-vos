...
| 17/9 | **Mi año se carga desde SIU Guaraní.** Quien tiene cuenta sube el «Reporte de materias - cursadas y finales» en PDF y se marcan solas las aprobadas (con nota y fecha) y las cursadas regulares (con el año). Se lee en el teléfono con pdf.js (`lib/leer-analitico.js`) y **el PDF no se sube**: tiene el DNI. Primero muestra lo leído y recién con «Cargar» guarda; no baja ningún estado; las optativas y lo que no está en el plan se muestran pero no se cargan. Probado con el analítico de Máximo: 24 aprobadas + 2 cursadas, cero renglones perdidos. **Y la regularidad vence a +4, no a +3:** el reporte dice 2024 → 31/3/2028; la app avisaba un año antes. La hoja ahora ofrece cinco años para elegir. | `2f3f3de` |

| 17/9 | **Sexta capa: peligrosidad de inundación** (ADA), separada de los cursos de agua para que quien solo quiera ver el arroyo no baje el archivo pesado. El dato sale de los **21 mapas de peligrosidad de La Plata** de `riesgohidrico.ada.gba.gov.ar` (uno por localidad, más Berisso por la cuenca Maldonado-Garibaldi; **Ensenada no está** en esa serie), que son Google My Maps y se bajan como KML. Son **42.427 polígonos, uno por manzana**: en un celular no entran. Se queda con **alta y media** (10.890) y deja afuera baja y muy baja a nula (31.537), que son la mayor parte del partido y harían ver toda la ciudad como inundable. El sur del mapa bajó a -35.25 porque Etcheverry llega hasta ahí. **La capa está en la app y el archivo NO**: dice «Todavía no está cargada». Falta correr `mapa/datos/herramientas/unir.ps1` hasta el final (unos 10 minutos) y dejar `peligrosidad.geojson`. Tres tropiezos anotados, los tres de PowerShell desarmando listas: se perdieron dos corridas de 10 minutos, una con 170.000 anillos de un punto | `bdffbbb` |

| 17/9 | **El panel no cargaba: quedaba en los esqueletos.** En el formulario de trámites, un comentario HTML (el de por qué el enlace es `type="text"`, del 6/9) tenía **comillas invertidas**, y ese formulario vive adentro de un texto de JavaScript que abre y cierra con comillas invertidas. La primera cerraba el texto antes de tiempo, el guion entero del panel daba `SyntaxError` y el navegador no corría nada: **no fallaba solo Trámites, se caía todo el panel**. Se cambiaron por «comillas latinas»; no cambia nada de lo que se ve. Se revisó la sintaxis de los guiones de todas las pantallas: **era el único**. **Regla: dentro del HTML que arma el guion, ni en los comentarios van comillas invertidas.** Lo de detrás del login no se pudo probar sin la cuenta del equipo. `v49`, para que quien tenía guardado el panel roto reciba el arreglado | `34496a2` |

| 17/9 | **Rediseño de tarjetas, etiquetas, acordeones y estados vacíos** (el `v48`, hecho en otra sesión y publicado recién después del arreglo del panel). **Dos fuentes y no cuatro:** salen Montserrat y Roboto Mono de las 19 pantallas; subtítulos, botones y datos pasan a Roboto (las fichas de Estudiemos cargan las suyas y no se tocaron). Las tarjetas tienen más aire, sombra suave, se aprietan al tocar y se levantan con mouse; los títulos dejan las mayúsculas. Las etiquetas pasan de pastilla negra a un tinte claro del color de la línea. Los acordeones se abren con animación. Los estados vacíos llevan ícono, título y una frase (Inicio y Agenda), y el flyer de una publicación va en un recuadro de alto fijo. **Un arreglo al subir:** la regla del acordeón apuntaba a `.lista`, pero en Trámites adentro va `.rejilla-fichas`: ahí se perdía el margen de abajo. Ahora es `> *`. Sale como **`v50`** y no `v48`: `v49` ya estaba publicado y con un número menor nadie recibe la hoja nueva | `e182490` |

| 18/9 | **Rediseño de la agenda (paquete «Perfecto mobile app design»), ronda 1.** Hoja nueva `estilos-rediseno.css`, cargada **después** de `estilos.css` en las 19 pantallas: papel blanco cálido en vez del amarillo suave, sombras suaves en vez de la segunda tinta, calendario de círculos (hoy = anillo, elegido = disco amarillo). En **Fechas**: el título de la tarjeta va primero, **insignia HOY roja** y cubo amarillo solo el día de hoy, banner negro **«Lo próximo»**, **chips para filtrar por línea** (filtran la lista, no el calendario), vacíos y esqueletos nuevos (el del calendario reserva el alto). **Dos choques resueltos:** la regla `.vacio` del paquete también agarraba los huecos del calendario antes del día 1 (`.cal-dia.vacio`) y la grilla se iba al doble del ancho → `:not(.cal-dia)`; y el banner se acotó a `#proximo` porque el inicio tiene su propio `.proximo`, callado a propósito. **No entró:** la campana de novedades (no hay de dónde sacar los avisos) y la **ronda 2** —Semana, época de parciales, faltas—, que pide las tablas `cursada` y `falta` y cuya `.hoja` pisaba las hojas de Carrera, Mapa y Trámites. `v51` | `d7d2423` |

| 18/9 | **Tablas de la ronda 2, creadas en Supabase** (`tabla-cursada.sql`, ya corrido). `cursada` (materia, día 1-7, bloque mañana/tarde/noche, hora, aula, total de clases; una clase por celda), `falta` (una fila por falta, **sin contador guardado**: «te quedan» sale de contar filas) y `preferencias` (modo época de parciales y categorías visibles, **nunca cero** por un `check`). Las tres son privadas por usuario, sin excepción para el equipo, y `anon` no tiene permiso. Las preferencias van en tabla propia y **no en `perfiles`**, para no tener que abrir la fila donde vive `rol`. Probado: la cuenta A ve lo suyo, otra cuenta no ve ni borra nada de A. **Todavía no hay pantalla que las use**: la vista Semana, el modo parciales y la hoja de faltas son el paso siguiente | `f5f922e` |

| 18/9 | **Ronda 2 en pantalla: Semana, época de parciales y faltas.** **La cursada se carga a mano en Mi perfil** («Mi cursada»: materia, día, hora, aula, clases del cuatrimestre; el bloque sale de la hora). Iba a salir del reporte de SIU Guaraní, pero **ese reporte no trae horarios**: solo aprobadas y regularidades. Si aparece un comprobante de inscripción con horarios en PDF, se le puede escribir un lector como `lib/leer-analitico.js`. En **Fechas**: pestañas Mes/Semana adentro del calendario; la Semana pone días en filas y mañana/tarde/noche en columnas, con la cursada y las fechas de la agenda **de un solo día** (un período de semanas llenaría todas las tardes y taparía los huecos). Tocar una materia abre la **hoja de faltas**: lo grande es lo que te queda, tope al 80%, «Falté hoy» agrega una fila y «Deshacer» borra la última. **Época de parciales**: un interruptor que deja visibles solo las categorías que elijas (nunca cero), afecta lista, calendario, «Lo próximo» y Semana; se guarda en el teléfono y, con cuenta, en `preferencias`. **La librería grande de Supabase se baja en Fechas SOLO si hay una sesión guardada** (`sb-…-auth-token`): quien entra sin cuenta baja lo mismo que antes. Dos nombres que chocaban: la hoja es `.cursada-hoja` (`.hoja` es la de Carrera/Mapa/Trámites) y la lista de Mi perfil es `.lista-cursada` (`.mi-cursada` ya era otra pieza, amarilla). Probado primero con un cliente falso en el navegador y después **por Máximo con su cuenta, contra la base: anda**. `v52` | `6c6d8c0` |

| 18/9 | **La campana de novedades.** Va en la cabecera de todas las pantallas, al lado del perfil, con un globo rojo que cuenta lo no leído. **Las novedades son las publicaciones de la agenda de los últimos 14 días**: no hay tabla de avisos aparte, así el equipo no carga nada dos veces. Qué está leído vive en el teléfono (`bolivar-novedades`: la fecha hasta la que se marcó todo y las abiertas sueltas), sin cuenta. El panel entra desde la derecha con fondo de vidrio; un aviso de «Info importante» lleva punto rojo; «Marcar todas como leídas» y Escape andan. El pedido sale cuando la pantalla ya cargó y se guarda cinco minutos en la pestaña. **La cabecera pasa a 88/1fr/88** (los costados iguales para que la marca siga centrada) **y sin separación**: con 10px entre columnas, «LA BOLÍVAR / CON VOS» se partía en tres renglones. **Ojo:** hoy la campana está vacía y es correcto: las 46 publicaciones se cargaron juntas el 27/8. Se va a llenar con lo que el equipo publique de acá en adelante. `v53` | `37bd514` |

| 18/9 | **Cátedras se corría de costado en el teléfono.** El culpable era un correo: `configuraciondeproblemassociales@yahoo.com` es una sola «palabra» más ancha que la tarjeta, y empujaba la página 16 px. `.lista-correl a` pasa a `overflow-wrap:anywhere` y se parte en dos renglones. Ojo al buscar estos desbordes: el chip «Gestión del Riesgo» también asoma a la derecha, pero vive en una fila con scroll propio y no corre nada. `v54` | `65940df` |

| 18/9 | **Dominio propio: `labolivarconvos.ar`, y la mudanza.** Desde algunas redes Wi-Fi (el proveedor o el router) **todo `*.vercel.app` da «no existe» en el DNS**: la app salía «sin conexión» y con los logos rotos, y con datos móviles andaba. Se registró `labolivarconvos.ar` en NIC, delegado a `ns1/ns2.vercel-dns.com`; en Vercel la principal es sin www (`www` redirige). **La dirección vieja NO se redirige desde Vercel**: el teléfono guarda los datos por dirección, y una redirección del servidor dejaba a la gente en la nueva sin plan, carrera ni código de respaldo (pasó unos minutos el 18/9 y se revirtió). La redirige `app.js` («La mudanza»): prueba que la nueva conteste, empaqueta lo `bolivar-*` en el `#` y del otro lado escribe solo lo que falta, **una vez y solo si el referrer es la dirección vieja** (si no, un link armado podría plantar un código de respaldo ajeno). No viajan la sesión ni las copias de pantallas. Aviso arriba «Nos mudamos». Los `og:image` apuntan al dominio nuevo. Supabase: la nueva en Redirect URLs; el **Site URL pasa a `https://labolivarconvos.ar/mi/`** cuando esto esté arriba. Las fichas no cargan `app.js`: un link viejo a una ficha sigue en vercel.app, y está bien. `v57` | `706a92a` |
| 18/9 | **Primera ronda de feedback de estudiantes (v58).** **La campana no abría en el inicio**: su panel era `id="novedades"`, el mismo id que la lista de novedades de la portada, y `abrir()` creía que ya estaba abierto; pasa a `panel-novedades`. **«Mandanos tu resumen» no se veía en tema claro**: tenía de fondo `--tinta-borde`, que el rediseño volvió beige, con letra amarilla; va con `--negro` (igual la insignia de las herramientas y el tilde de «leído» en las fichas, que tenían el mismo problema). **El chat ofrece desde el saludo «Te pasamos con un compa de la Bolívar»**, no solo cuando no encuentra. **Los PDF del tramo optativo y del plan de Fono no abrían en el celular**: estaban cargados con `http://`, y Chrome de Android, que descarga los PDF en vez de mostrarlos, bloquea las descargas inseguras desde una página segura. Se pasaron a `https://` los cuatro enlaces así en `tramites` (29, 48, 49, 51), el plan de Fono pasó al que publica hoy la facultad (`2026/02/plan-de-estudios-de-lic-en-fonoaudiologia-1.pdf`), y **el panel guarda los enlaces con https** aunque se peguen con http. **El buscador de Info útil busca palabra por palabra**: «plan fono» o «resolución tramo optativo» daban cero. En la base: tilde de «Bolívar» en el titular de Quiénes somos y otros tildes del texto del Consejo. **Pendiente: los logros del Consejo** (la línea de tiempo existe, `pagina_consejo.logros` está vacío: los tiene que escribir la agrupación) | `ed1f29c` |
| 19/9 | **Rediseño de Estudiemos** (Máximo: «todo antes del 21», con plan B). **La portada ya no es un riel de tarjetas iguales para todos**: arriba, **«Seguí donde dejaste»** (lo último que abriste, cuánto leíste y un botón que te devuelve al mismo punto de la página); después **tres modos por lo que vas a hacer** —**Repasar** (fichas, cuadro de modelos, videos), **Practicar** (Fonoteca, 3D, finales) y **Territorio**—, en grilla y no en riel, **ordenados según la carrera** (la Tecnicatura arranca por Territorio, TS lo tiene segundo, Fono último). **Territorio** es un esquema propio del mapa de riesgo: 170 arroyos y canales simplificados y 188 barrios RENABAP (`estudiemos/territorio.js`, 7,5 KB, sacado de `mapa/datos/`), con los reportes **aprobados** que se traen de `reportes_riesgo`; el agua se dibuja al entrar en pantalla, los puntos aparecen y los seis más nuevos laten. Con cero reportes la fila de conteos se esconde y el pie invita a cargar el primero. **El buscador encuentra fichas y herramientas**, no solo apuntes (lee `fichas/index.html`, así una ficha nueva aparece sola). El material compartido **baja al final**. **La huella vive solo en el teléfono** (`estudiemos/huella.js`, localStorage `bolivar-huella`): la cargan las fichas por `ficha.js`, el cuadro, Fonoteca, videos, 3D y mapa. Una voz nueva, **Roboto Mono, solo para datos** (cuántas, cuándo, cuánto leíste). **Plan B: `estudiemos/anterior.html` es la pantalla vieja entera**; para volver, copiarla encima de `estudiemos/index.html`. **Falta probar en teléfono real.** `v62` | `a269767` |
| 19/9 | **Los videos «En un minuto» suben arriba del mapa** (pedido de Máximo). Dejan de ser una tarjeta de Repasar y pasan a una **fila de portadas verticales** entre «Seguí donde dejaste» y los modos, así que quedan antes que Territorio en las tres carreras. Los de tu carrera primero (la materia se busca en los planes), sin etiqueta: con 5 de 7 de Fono el cartel era ruido. Cada portada abre el reproductor en ese video (`videos/#v=`), y **el buscador encuentra cada video** por título y materia. Es el único deslizable de la pantalla, a propósito. `v63` | `5ba43b8` |
| 19/9 | **Arreglos de la revisión nocturna** (hechos en la rama `arreglos-noche-19-9` y **subidos a main el 19/9 a la mañana**). **El buscador de Estudiemos perdía las cuatro fichas de `leer/`** (Anatomo 3 a 6): guardaba solo la ruta, sin `?f=`, así que la primera abría «No encontramos la ficha» y las otras tres se descartaban por repetidas. **La pantalla de videos no definía `window.RAIZ`**: `app.js` registraba `/estudiemos/videos/sw.js` (404), y quien entraba por primera vez desde el link de un video se quedaba sin service worker. **`behavior:'instant'` con respaldo** en el reproductor y en «Seguí donde dejaste»: Safari anterior a 16.4 no lo conoce y tira error. **El registro ya no anota las transiciones que cancela el navegador** (`@view-transition`): del 17 al 19/9 eran 57 de los 63 errores y gastaban el cupo de tres por visita. Informe completo en `INFORME-NOCHE.md` (fuera de git: tiene detalles de la base; **quedó solo en la otra computadora**). `v64` | `b4d3d18` · `faf2d0a` |
| 19/9 | **Revisión en producción antes del teléfono, y «Lo próximo» de Fechas.** Se recorrieron las pantallas en `labolivarconvos.ar` a 375 px (inicio, Fechas, Dónde curso, Estudiemos, videos, cuadro de modelos, Mi año, Mi perfil, Cátedras, Info útil, mapa, panel, Fonoteca, Consejo, Trayecto): sirve `v64`, ningún error en consola, ningún pedido fallido, nada se corre de costado; el buscador encuentra las fichas de `leer/` con su `?f=` y el video abre en el suyo. **Lo único roto: el banner «Lo próximo» de Fechas** (el del rediseño, `v51`) **repetía el error que el inicio tenía hasta el 5/9** (`de92a32`): ordenaba por `fecha_desde` todo lo que no terminó, así que ganaba «Desarrollo de seminarios» (18/8 al 21/11) y decía «Hoy» tres meses, tapando lo que venía; el 21 iba a esconder «Día del estudiante». Ahora es la misma regla que el inicio: primero lo que no empezó y, si no queda nada, lo que está en curso y termina antes, con «Hasta el …». Probado en local con los 46 datos reales contra cinco fechas. **La insignia HOY de cada tarjeta se deja**: va al lado del rango de fechas y se entiende. Sin `v` nueva: es una pantalla | `5bb9c3d` |
| 19/9 | **Mazo de repaso de Antropología de la Otredad.** 31 tarjetas de las cinco unidades de Teorías de la Cultura (215 B), materia común a **TS y Fono**: está en las dos listas de Fichas, es herramienta de Repasar en Estudiemos con «Sabés n de 31», y la huella lo anota como Mazo. Vino como **export standalone** de Claude Design, no como `.dc.html`: la plantilla se sacó de adentro del paquete y los clics pasaron de `sc-camel-on-click` a `onClick`, así corre con `ficha.js` **sin tocar el motor**. Por eso no hay `v` nueva: con el motor cambiado, quien ya tenía `ficha.js` guardado vería el mazo roto en la primera carga. Suma tema oscuro. Probado en local a 375 px, claro y oscuro. | `ca5f02e` |
| 19/9 | **El rótulo «Herramienta» del buscador de Estudiemos pisaba la descripción (v65).** Visto buscando «fonoteca» y «otredad» a 375 px. La columna del `.tipo` en `.est-hallazgo` medía `9ch`, y «HERRAMIENTA» son 11 letras con espaciado: se salía y se montaba sobre el nombre. Ahora mide `calc(11ch + 11 * .06em)`, lo justo para el rótulo más largo, y no se parte; los nombres siguen alineados de fila a fila. Los demás rótulos (Ficha, Video, Tríptico, Fonoteca, PDF…) entran. La lista de «Seguí donde dejaste» tiene su propio `.tipo` y **queda en `9ch`**: su rótulo más largo es «Fonoteca». **Sube `v`** porque `estudiemos.css` se sirve de lo guardado: sin número nuevo, la primera visita después de subir seguía mostrando el rótulo encimado. Probado en local a 375 px escribiendo en `#filtro`. **Probado en producción el 19/9** (`labolivarconvos.ar`, 375 px, «fonoteca», «otredad», «minuto»): en la segunda carga la columna mide 76 px, «Herramienta» entra justo y quedan 12 px hasta el nombre. La **primera carga salió todavía encimada** (57 px, el ancho viejo): el navegador ya tenía la v64 guardada y esa sirvió la hoja vieja mientras se instalaba la v65. Es lo esperado y confirma lo del día del lanzamiento: lo nuevo llega en la segunda carga. | `172a148` |
| 19/9 | **Línea de tiempo de Historia Social.** Once procesos de 1880 a 2001 de Historia Social de América Latina y Argentina (214, primer año de **TS**), en `estudiemos/fichas/historia-social/`: cada hito se abre con la síntesis, la bibliografía y una pregunta; filtro por categoría y «Abrir todo». Está en Fichas (Trabajo Social), es herramienta de Repasar en Estudiemos, sale en el buscador y la huella la anota como **Línea** (no «Línea de tiempo»: la columna de «Seguí» mide 9 letras). Vino como **export standalone**, igual que el mazo, y corre con `ficha.js` **sin tocar el motor**. Tres arreglos al diseño: **tocar un hito no hacía nada** (la plantilla llamaba a `n.onClick` y el código nunca lo definía); los `ref` pasaron a `id`, porque el motor no los entiende y además redibuja todo; y como cada toque redibuja, los hitos que ya entraron llevan `.visto` para no volver a animarse. Los hitos aparecían solo con `IntersectionObserver`: tienen **respaldo por posición al bajar**, porque en el panel de vista previa (y quizás algún webview) quedaban invisibles. Tema oscuro con `--tinta` como el mazo. Sin `v` nueva: la tarjeta de la portada llega en la segunda visita. Probado en local a 375 px, claro y oscuro. **Probado en producción el 19/9** (`labolivarconvos.ar`, 375 px): la línea abre, filtra y aparece sin errores de consola ni desborde. En la **primera carga** después de subir, la tarjeta no estaba en Repasar y la huella anotó la visita como «Ficha»: el service worker sirvió el `portada.js` y el `huella.js` guardados. En la **segunda** ya estaba todo: la tarjeta «De tu carrera», el buscador, y la visita anotada como Línea (el renglón viejo se pisa, no queda nada mal anotado). | `6b7436e` |
| 19/9 | **Repaso de Introducción a la Psicología.** Las cuatro partes del programa (bloque introductorio, unidad 1, unidades 2 y 3, unidad 4), materia común a **TS y Fono**, en `estudiemos/fichas/psicologia-introduccion/`: por unidad, conceptos, un cuadro comparativo, 22 fichas rápidas para dar vuelta y 10 preguntas de autoevaluación con su porqué, y qué sirve en la práctica de cada carrera. Está en las dos listas de Fichas (rótulo **Repaso**), es herramienta de Repasar en Estudiemos con «Respondiste n de 10» y sale en el buscador; la huella la anota como Ficha. Vino como **export standalone**, igual que el mazo y la línea, y corre con `ficha.js` **sin tocar el motor**. Arreglos al diseño: `sc-camel-on-click` pasa a `onClick`; el `key` de la unidad no existe en el motor y, como cada toque redibuja todo, dar vuelta una tarjeta hacía entrar de nuevo la unidad entera y responder reanimaba todas las devoluciones: ahora cada animación va solo en el dibujo que le toca. Las respuestas **se guardan en el navegador** (`bolivar-fichas-psicologia`; en el diseño se perdían al salir). Cambiar de unidad desde abajo vuelve al principio de la unidad nueva. Tema oscuro con `--tinta`. Sin `v` nueva. Probado en local a 375 px, claro y oscuro. | — |
| 19/9 | **La grilla «EN INSTAGRAM» del inicio se puede llenar: pestaña 📸 del panel.** Estaba construida a medias desde el 4/9 y **nunca se mostró**: la tabla `posteos_ig` existía vacía, la función `traer-instagram` quedó escrita y **sin nadie que la llame** —`pg_cron` y `pg_net` no están instalados en el proyecto—, y como el inicio esconde la sección cuando no hay filas, no se veía ni el hueco. **Se decide cargar a mano y no conectar la API de Meta**: la que servía (Instagram Basic Display) está dada de baja desde diciembre de 2024, y la que la reemplaza pide cuenta de empresa, app revisada y un token que **vence cada 60 días**, o sea una sección que un martes desaparece sola y nadie se entera. Cargando a mano, además, la agrupación **elige** qué placa va, que para el 21 es mejor que «las últimas seis». Si algún día la función automática arranca, escribe en la misma tabla y esto no se tira (ojo: el `id` de la API es el número del posteo y el del panel es el código del link, así que el mismo posteo por los dos caminos entraría dos veces). **El `id` sale del link** —`/p/ESTO/`— así que cargar dos veces el mismo posteo lo pisa en lugar de repetirlo, y el reconocedor acepta las dos formas que da Instagram: la de «Copiar enlace» y la de la barra del navegador, que mete el usuario en el medio (`/SimonBolivarfts/p/…`), más reels y tv. **Lo que costó encontrar: las políticas no alcanzaban.** La tabla se había creado para que la llenara solo la función automática con la llave de servicio, así que `authenticated` tenía **únicamente lectura** y PostgREST cortaba con «permission denied» antes de mirar ninguna política: faltaba el `grant insert, update, delete`. A `anon` no se le da escritura, así que quien no entró con cuenta queda afuera por los dos lados. El campo del texto **no es el epígrafe de Instagram con los hashtags**: es lo que escucha un lector de pantalla, porque una placa es texto adentro de una foto y sin eso el posteo no existe para esa persona. La placa se sube al mismo depósito y con la misma función que las fotos de «Quiénes somos». **Sin `v` nueva, y esta vez no se llega recién en la segunda carga**: el panel es una pantalla y las pantallas se sirven **primero de la red**, así que la pestaña está en la primera. `tabla-posteos-ig.sql` **aplicado en Supabase** y probado con transacciones deshechas: el equipo carga, corrige y saca; un estudiante lee pero no carga (lo frena la política); sin cuenta ni siquiera llega (lo frena el permiso). Producción quedó en 0 filas. Las dos vistas nuevas, probadas con un banco de pruebas de 29 comprobaciones (lista vacía y con datos, links que no sirven, cambiarle el link a un posteo ya cargado, reemplazar la placa, sacarlo, y que no se cuele HTML en el texto). **Falta cargar el primer posteo de verdad**: eso lo hace la agrupación desde el panel | `—` |
| 19/9 | **Las placas de Instagram entran al río de novedades (v66).** Máximo: «la sección se pierde con el fondo, aparece muy chica y no tiene info». Las tres cosas eran ciertas y había una cuarta que no se veía: la grilla metía con `object-fit:cover` una placa **4:5 dentro de un cuadrado**, o sea que le comía el 20 % arriba y abajo, y en una placa —texto adentro de una imagen— eso corta renglones enteros. Se armaron cuatro variantes en un lienzo aparte y eligió la **miniatura al costado**: la placa deja de ser una grilla propia y pasa a ser **una `.tarjeta` más adentro de Novedades**, ordenada por fecha junto con lo escrito. No hay sección «EN INSTAGRAM»: como la agrupación publica allá casi todos los días y en la app cada tanto, la placa queda arriba **por reciente y no por tener lugar reservado**. Tope de **dos placas por río** (y cuatro tarjetas en total): sin tope, un fin de semana activo en Instagram empuja abajo la única novedad escrita, que suele ser la que importa. La consulta de `posteos_ig` viaja en el mismo `Promise.all` que las otras cuatro, así que **el río no espera ni una décima**, y queda **afuera del control de errores**: una novedad escrita que no llega es un error, una placa que no llega no. El enlace al perfil, que vivía en el título de la grilla, baja al final como «Seguinos en Instagram»: era el único del inicio. **Dos correcciones que salieron de probar corriendo.** La primera: el CSS lo escribí en `estilos.css` y **no ganaba ninguna regla**, porque de las tarjetas manda `estilos-rediseno.css`, que carga después; medido en el navegador, el `h3` seguía en 19 px y la sombra dura se volvía blanda. De paso quedó claro que el rediseño **ya había sacado el canto de color de 4 px** de todas las tarjetas a propósito («la línea editorial la dice la etiqueta pastel»), así que la placa tampoco lo lleva: Instagram no es una línea editorial, no es un tema, es una puerta. La segunda: el epígrafe **viene en un solo párrafo**, sin saltos de línea, así que cortar por `\n` devolvía los 434 caracteres y el recorte caía a mitad de palabra («…Ya está el repas»). Ahora corta por la primera oración, y **no por el primer punto**: eso dejaba «Dr. Pérez da la charla» en **«Dr.»**, y lo mismo con Lic., Prof., Av. o una hora escrita 18.30; se pide que la frase llegue a 30 caracteres. La placa va con `object-fit:contain` en un recuadro fijo de 130×162: entra entera, nunca recortada, y el hueco está reservado desde el primer cuadro para que la tarjeta no salte al llegar la foto. El `alt` de la imagen **queda vacío** a propósito: el texto ahora se lee en el titular, y repetirlo haría que un lector de pantalla lo diga dos veces. **Sube `v` a 66**: el inicio es una pantalla y llega fresco por la red, pero `estilos-rediseno.css` se sirve de lo guardado, y sin número nuevo la primera visita dibujaría las tarjetas nuevas con las reglas viejas. Probado corriendo a 375 px, **claro y oscuro**, con el posteo real: placa 130×162 sobre un original de 1080×1350, tarjeta de 343×200, sin errores de consola. El recortador de frases, con diez casos (abreviaturas, horas con punto, sin punto final, emojis, vacío). | `—` |
| 19/9 | **Los carruseles se pueden cargar enteros.** Máximo preguntó si era posible y eligió subir todas las placas, no escribir el número a mano. Una parte ya andaba sin tocar nada: un carrusel tiene la misma dirección que un posteo simple, así que cargando la portada el link abre la serie entera en Instagram (ojo: copiar el link **desde la primera placa**, porque si se copia mirando la tercera queda `?img_index=3` y a la gente le abre ahí). Lo que faltaba era guardarlas y avisar que hay más. **`imagenes text[]`** guarda la serie en orden; **`imagen` sigue siendo la portada y no se tocó**, que es lo que lee el inicio y lo que escribiría la función automática si algún día arranca. El posteo que ya estaba cargado pasó a ser un carrusel de una. El panel sube las placas **al elegirlas y no al guardar**, igual que las fotos de «Quiénes somos»: se ven al instante y el orden se arregla mirando, con un ↑ por placa y la primera marcada como portada. Sube **de a una y esperando** —diez en paralelo desde el wifi de la facultad es la forma más segura de que fallen la mitad— y si se corta a mitad de camino **las anteriores quedan cargadas** y lo dice. Tope de **10**: Instagram permite 20, pero diez ya es larguísimo y el tope evita que una elección equivocada suba media galería. `tipo` pasa a `CAROUSEL_ALBUM` cuando hay más de una, que es el vocabulario de la API. Sacar el posteo borra **todas** sus placas del depósito, incluidas las que se agregaron en esa pantalla sin llegar a guardarse. **Una decisión de diseño que se dio vuelta al verla.** El contador iba como insignia negra «1/5» arriba a la derecha de la placa, que es la convención de Instagram; se dibujó y **tapaba «Intro a la Filo»**, el titular de la placa. Eso contradice lo que se decidió el 19/9 al sacar el recorte cuadrado: si no se recorta una placa para no cortarle palabras, tampoco se le tapan con una insignia. El contador bajó al renglón de datos, **«19 SEP · 5 PLACAS ↗»**, que no tapa nada y además dice qué vas a encontrar del otro lado. **En el inicio se sigue viendo solo la portada**, y las otras se guardan igual: el día que se muestren adentro de la app no hay que volver a cargar nada. No se hace un visor propio —la miniatura mide 130 px, Instagram ya lo hace bien y, sobre todo, **cada placa es texto adentro de una imagen**: mostrar cinco obliga a escribir cinco descripciones por posteo o esas cuatro no existen para quien usa lector de pantalla, y eso nadie lo sostiene—. **Sin `v` nueva**: no cambió ni `estilos-rediseno.css` ni `sw.js`, todo quedó en dos pantallas, que se sirven primero de la red y llegan en la primera carga. Probado corriendo a 375 px, claro y oscuro, y con un banco de 22 comprobaciones (carrusel de tres, una sola, ninguna, el tope de 10, la red cortándose a mitad de subida, editar uno que ya existe, un posteo viejo sin lista, y que sacar borre todas sin repetir). | `—` |
| 19/9 | **Instagram vuelve a tener sección propia: una fila que se desliza y da la vuelta, más un visor (v67).** Pedido de Máximo: «un swipe infinito, con todas las publicaciones hacia los costados», y las dos cosas —la fila con todos los posteos y las placas de cada carrusel deslizables adentro de la app—. **Esto deshace la fusión de esta misma tarde**: con una fila propia, el posteo más nuevo aparecía dos veces, en la fila y en el río, así que las placas salen de Novedades y vuelven a `EN INSTAGRAM`, arriba del río porque es lo que más se mueve. Novedades queda solo con lo escrito, como estaba antes. **«Infinito» es dar la vuelta, no moverse solo**: lo que se mueve solo no se puede leer, y una placa es para leer. La vuelta se hace dibujando la fila **tres veces** y arrancando en la copia del medio; cuando el dedo se acerca al borde de una copia se salta un ancho entero de golpe, sin animación, y como las tres son idénticas no se ve nada. Así queda intacto el deslizamiento del sistema —dedo, rueda, teclado, barra— sin reimplementar ninguno. Con menos de tres posteos no se dan vueltas: serían dos tarjetas iguales mareando. El visor usa la misma `.hoja` que Info útil, el mapa y Mi año: las placas se deslizan de a una con `scroll-snap`, hay un contador «Placa 2 de 4» con `aria-live` —para quien no ve las placas es lo único que avisa que algo se movió, porque hay un epígrafe por posteo y no uno por placa— y **es el único lugar de la app donde el epígrafe se lee completo**. Cierra con la ✕, con Escape y tocando el fondo, y el foco vuelve a la tarjeta desde la que se abrió. **Dos errores encontrados probando, no leyendo.** El primero: la fila arrancaba pegada al principio en vez de en la copia del medio, porque un solo `requestAnimationFrame` no alcanza para que esté medida; se reintenta hasta que tenga ancho real. Al arreglarlo metí un segundo error —exigía que la posición quedara exacta, y el `scroll-snap` la corre unos píxeles, así que la vuelta **nunca se enganchaba**—; se sacó la exigencia. El segundo, más grave: **la página entera se corría de costado, 921 px**. El culpable era el nombre accesible de cada tarjeta, que va en `.solo-lectores` con `position:absolute`: sin un ancestro posicionado se ubica respecto de la PÁGINA, se escapa del riel y la estira. Con eso la hoja del visor se dibujaba torcida, medio corrida fuera de la pantalla. `.ig-tarjeta` pasa a `position:relative`. Es el mismo desborde que tuvo Cátedras el 18/9, por otra causa: **conviene mirar los otros rieles de la app que usen `.solo-lectores` adentro.** **Lo que NO se pudo probar:** que el salto de la vuelta se dispare con un dedo de verdad. El panel de vista previa **no entrega ni un evento `scroll`** —se midió: la posición cambia y llegan cero eventos, ni con asignación por código ni con rueda—, así que la vuelta quedó probada solo en su aritmética, con la función real y un riel de mentira en Node (ocho casos: el medio, los dos bordes, los dos límites exactos, y un riel sin medir). En un navegador de verdad el evento siempre llega; igual **es lo primero a mirar en el teléfono**, y si falla se apaga en una línea (`daLaVuelta = false`) y queda una fila común. Lo demás sí probado corriendo a 375 px con los tres posteos reales que cargó Máximo, claro y oscuro: la fila arranca en el medio, la página no se corre (0 px), el visor abre centrado con las cuatro placas cargadas, el contador, el epígrafe entero, y cierra por los tres caminos devolviendo el foco. Contraste del pie de la tarjeta en oscuro: 8,52. **Sube `v` a 67** porque las reglas de la fila viven en `estilos-rediseno.css`, que se sirve de lo guardado: sin número nuevo, la primera visita dibujaría las portadas apiladas una abajo de la otra. | `—` |
| 19/9 | **Los avisos al celular: la alarma de mesas deja de depender de que alguien abra la app (v68).** El agujero era viejo y grande: la ventana para anotarse dura cuatro días y la alarma de la portada **solo la ve quien entra**. Ahora se prende un interruptor en **Mi perfil** y el aviso llega con la app cerrada. Tres canales que se prenden por separado —**mesas**, **novedades**, **mis fechas de final**— porque no todo el mundo quiere lo mismo, y apagar los tres de una es lo que hace que se pierda el único que importaba. **Cuatro piezas y ninguna sirve sola:** el interruptor (`pintarAvisos` en `app.js`), el timbre guardado (`tabla-avisos.sql`), quien manda (`supabase/functions/avisos/index.ts`, una vez por hora) y quien dibuja (oyente `push` en `sw.js`). **Las decisiones que no son obvias.** (1) El permiso **no se pide al entrar**: un cartel del sistema que aparece sin que nadie lo pidiera se cierra por reflejo, y en Chrome ese «no» es para siempre. Se pide recién al tocar el interruptor. (2) `requestPermission()` va **antes de cualquier `await`**: Safari solo lo deja pedir mientras dura el toque, y esperar al service worker primero hacía que el cartel nunca apareciera. (3) Se anota que el aviso salió **antes** de mandarlo, no después: el reloj corre cada hora y «mañana cierra la inscripción» saldría veinticuatro veces. (4) **Nadie puede leer `avisos_suscripciones`**, ni con sesión: un endpoint ajeno es un timbre abierto. Se entra por dos funciones `security definer` que escriben y no dejan leer. (5) Nada entre las 21 y las 9: un teléfono que suena a las cuatro de la mañana se apaga para siempre al otro día. (6) Las novedades **vencen a las 48 horas** de marcarlas (`avisar_at`, lo pone un disparador): sin eso, quien prende los avisos en marzo recibe de golpe lo que alguien marcó en octubre. (7) En iPhone hace falta la app instalada —límite de Safari—, así que ahí la tarjeta, en vez de decir que no se puede, **ofrece instalarla** reusando `invitarAInstalar()`. La tarjeta y el switch son los mismos de «Época de parciales»: dos interruptores que hacen lo mismo tienen que verse igual, así que **no hay CSS nuevo**. **Sube `v` a 68, y esta vez no es por estilo**: el service worker viejo no tiene el oyente `push`, o sea que un teléfono que se quede en la v67 puede suscribirse igual y no mostrar nunca lo que le llega. Probado corriendo a 375 px y en escritorio: la tarjeta en sus tres estados (apagada, prendida y **bloqueada** —el navegador de la vista previa tiene las notificaciones denegadas de fábrica, que resultó ser una buena forma de probar ese camino—), el guardado local, y la regla de que no se pueden apagar los tres canales. **Lo que NO se pudo probar acá y hay que mirar en el teléfono:** el envío de punta a punta, porque el navegador embebido no da el permiso. **No anda hasta que se hagan los tres pasos en Supabase** que están en «Lo que depende de Máximo». | `—` |
| 24/9 | **`PROMPT-DISENO.md` al día con el rediseño.** El molde que se le pega a una IA antes de pedir una pantalla seguía describiendo la app de antes del 17/9: sombra dura `2px 2px 0`, cuatro fuentes con Montserrat y Roboto Mono, radios de 10 y 6, y `--tinta-borde` como si fuera negro (el mismo malentendido que dejó invisible «Mandanos tu resumen» en `v58`). Ahora dice lo que sirve `estilos-rediseno.css`: fondo blanco cálido, `--superficie-2`, dos sombras (`--sombra-1` reposo, `--sombra-2` lo que se levanta), radios 14/10/20, dos voces (Archivo Black y Roboto; Roboto Mono solo en la portada de Estudiemos), etiquetas pastel `--et-*`, el amarillo también en el chip activo y el día elegido, y que el CSS nuevo va en `estilos-rediseno.css` porque en `estilos.css` puede no ganar. Solo documentación: sin `v` nueva | — |
| 24/9 | **Las 17 fichas de texto son una pantalla más de la app (v73).** Salió de la propuesta de diseño de Estudiemos (variante A): las fichas de Anatomo y DFOF tenían un sistema propio de Claude Design —Archivo, letra de terminal, «M1·ANQ·11», `~/fichas/... $ open`, sombras duras— y **no tenían modo oscuro ni la barra de la app**. Ahora cargan `estilos.css` y `estilos-rediseno.css`, tienen cabecera, barra de abajo y oscuro, y la plantilla (una sola, igual en las diecisiete) usa `.titulo-pantalla`, `.tarjeta`, `.chip`, `.etiqueta` y `.titulo-seccion`. Arriba: la materia, el título, la **entrada** (que el diseño viejo no mostraba), la fuente y una barra de progreso de las preguntas; debajo, un **índice pegado** (Qué es · Cómo funciona · Preguntas · Falta). Las piezas van **siempre abiertas** y no en acordeón: el motor redibuja todo en cada toque y un acordeón se cerraría solo al responder. Las respuestas se marcan con celeste hondo (bien) y rojo (mal). **Se deja afuera el ESQUEMA MENTAL**, igual que en `leer/`: son instrucciones de dibujo para Claude Design («al presionar el botón 1 se muestra en rojo…») y no hay dibujo. `ficha-texto.js`: pinta cabecera y pie, los estilos sueltos pasan a clases, y la materia de Anatomo pierde la cita del cuadernillo pegada. **Ni el parser, ni `ficha.js`, ni los `.txt` se tocaron.** Cada `index.html` bajó de ~16 KB a ~6 KB. Sube `v`: la hoja y la lógica se sirven de lo guardado. Probado en local a 375 px, claro y oscuro: las 17 cargan con sus 8 preguntas y sus 2 máquinas, sin errores de consola ni desborde. **Falta probar en teléfono real.** | — |
| 24/9 | **Las fichas de texto dicen qué te falta (variante C sobre la A, misma v73).** Debajo del título, un bloque negro con tres cifras —**partes vistas**, **preguntas bien**, **sin responder**— y **un solo botón amarillo** con lo que más sirve hacer ahora: «Probate con las 8 preguntas» la primera vez, «Seguí con las 5 que faltan» después (te lleva a la primera sin responder y le pone el foco), «Mirá por qué erraste 2» cuando están todas y alguna mal, y «Volver a hacer las preguntas» si están todas bien. Abajo, «Seguir leyendo: …» con la primera parte que no viste. Al final de las preguntas, «Borrar mis respuestas». **Se guarda en el teléfono, por ficha** (`bolivar-ficha-<nombre>`, lo mismo que hace el repaso de Psicología) y no en la huella: la huella anota la visita, esto es lo de adentro. Se guarda también cuántas preguntas tenía la ficha: si el texto cambia de cantidad, las respuestas viejas se descartan. **Una parte cuenta como vista cuando estuvo en pantalla**, no cuando se pasó por encima: en la primera prueba, «Probate» saltaba hasta las preguntas y daba por vistas Qué es y Cómo funciona; ahora mientras dura el salto no se mira nada y además la parte tiene que estar a la vista. Probado en local a 375 px, claro y oscuro: primera visita, tres respuestas, recarga (quedan), las ocho, borrar; sin errores de consola. | — |
...
**Lo que pasó el viernes 18 (anotado el sáb 19).** No fue solo prueba: entraron de **`v55` a `v64`**. Una parte salió de probar (la primera ronda de feedback de estudiantes, `v58`) y otra son pantallas nuevas: Dónde curso, los videos «En un minuto», el cuadro de modelos y el rediseño de Estudiemos. **Ninguna de esas pantallas figura probada en un teléfono real**, así que el sábado arranca por ahí, con la misma regla: lo que falle y no se arregle chico, se vuelve atrás (Estudiemos tiene su plan B en `estudiemos/anterior.html`). La lista de posteos queda para después de esa prueba.
...
- **Se publica sin preguntar** (decidido el 3/9): no hace falta pedir permiso
  ...
  leyó**. Hay dos redes, y conviene usar las dos: levantar el servidor de
  `HttpListener`, abrir la pantalla y mirar consola y comportamiento; y
  **correr la lógica en Node** (esta máquina tiene `node v24.19.0` y
  `python 3.14.7`, aunque acá dijera que no hasta el 20/9/2026). Node es la
  única forma de probar el panel y Mi perfil **sin loguearse**: se copia la
  función a un archivo aparte y se le arma un `document`, un `db` y un
  `localStorage` de mentira. Así se probaron las vistas de Instagram y, el
  20/9, las 92 comprobaciones de los avisos. Lo que no se pueda verificar así se comitea, se avisa y **no
...
- Las limitaciones de esta máquina (heredocs que se rompen, servidor de prueba
  con `HttpListener`, el panel de vista previa que no dibuja con la ventana
  minimizada ni entrega eventos `scroll`) están en la memoria del proyecto, no
  acá. **Node sí hay**, ver arriba.
- **La base se puede mirar desde acá.** El conector de Supabase está autorizado
  para el proyecto `kcgewgelfgyndbszfdep`. Para leer, directo. Para **ensayar un
  SQL sin dejarlo puesto**, mandarlo envuelto en `begin; … rollback;` con un
  bloque `do $$ … $$` que levante `raise exception` si algo no cumple, y
  **después verificar con un `select` que no quedó nada**. Lo que el conector no
  puede: Authentication (SMTP, Site URL) ni los secretos de Edge Functions.
