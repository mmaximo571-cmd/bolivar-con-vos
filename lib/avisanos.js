...
window.Avisanos = (function(){
  ...
  /* El cartel de abajo dice qué se encuentra al tocar. Eran dos casos
     —«Paso a paso» o «Sitio oficial»— y «sitio oficial» terminaba
     puesto encima de cosas que no son ni un sitio ni de afuera: una
     pregunta frecuente, una ficha sin enlace, y desde el 6/9/2026 la
     de cátedras, que lleva a `/catedras/`. Misma regla que en Info
     útil: si el enlace no empieza con http, del otro lado está esta
     misma app. */
  function marcaDe(t){
    if (Array.isArray(t.pasos) && t.pasos.length) return 'Paso a paso';
    if (t.link_url && /^https?:\/\//i.test(t.link_url)) return 'Sitio oficial';
    return 'En la app';
  }
  ...
  async function traer(){
    ...
    try {
      const [rT, rF, rC] = await Promise.all([
        db.from('tramites').select('id,titulo,resumen,palabras_clave,pasos,link_url').eq('publicado', true),
  ...
  /* WhatsApp necesita el número ENTERO: país, el 9 de celular, área y
     línea. `wa.me/2214948957` no abre ningún chat —WhatsApp contesta
     «el número no es válido»—, y así estaba cargado el único contacto
     real que había: el enlace del traspaso llevaba a un cartel de
     error, que es exactamente lo que esta función existe para evitar.

     Se completa lo que falta en vez de dejarlo roto:
       10 dígitos (área + línea)   -> se le pone 549 adelante
       12 empezando en 54          -> le falta el 9 de celular
     Cualquier otra cosa se manda como vino: un número de otro país no
     se adivina. Si algún día hay contactos de afuera, esto se saca y
     se carga el número entero, que es lo que corresponde. */
  function numeroWhatsapp(destino){
    const d = String(destino).replace(/\D/g, '');
    if (d.length === 10) return '549' + d;
    if (d.length === 12 && d.slice(0, 2) === '54') return '549' + d.slice(2);
    return d;
  }

  function enlaceDe(c){
    if (c.tipo === 'whatsapp') return 'https://wa.me/' + numeroWhatsapp(c.destino);
  ...
  async function guardarConsulta(texto){
    try {
      /* Los otros dos también se recortan acá. `carrera` sale de
         localStorage y `pantalla` de la URL: los dos los controla el
         visitante. La base ya los rechaza si se pasan de largo; esto
         es para que a nadie le falle por un dato viejo guardado en su
         teléfono. */
      await db.from('consultas').insert({
        texto: texto.slice(0, 500),
        carrera: carrera ? String(carrera).slice(0, 20) : null,
  ...
  function fichaHTML(f){
    ...
    return `<a class="chat-ficha" href="${url}">
        ...
        <span class="marca-tipo ${f.marca === 'Paso a paso' ? 'guia' : 'enlace'}">${
          esc(f.marca)}</span>
      </a>`;
  }
  ...
  function preguntarCarrera(){
    ...
    d.querySelectorAll('[data-carrera]').forEach(b => {
      b.onclick = () => {
        ...
        /* Decirle la carrera al chat es decírsela a la app. */
        if (typeof anotarCarreraApp === 'function') anotarCarreraApp(carrera);
      ...
      decir('bot', (encontradas.length === 1
        ...
        `<div class="chat-fichas">${encontradas.map(fichaHTML).join('')}</div>
         <span class="chat-nota">¿No era esto? Escribilo de otra forma, o
           <button class="chat-enlace" data-pasar="1">te pasamos con un compa de la Bolívar</button>.</span>`);
  ...
  /* PREGUNTAS, NO TEMAS. Antes decían «Boleto universitario», «Becas»:
     un índice de rubros, que es lo que uno ya vio en Info útil. Escritas
     como las diría una persona, hacen otra cosa: muestran QUÉ SE PUEDE
     preguntar acá, que es justo lo que no se sabe frente a un campo
     vacío.

     Ojo antes de reescribirlas: el buscador puntúa por palabra clave y
     además da puntos por la frase entera, así que una pregunta más
     larga podría bajar de resultado. Se midió con los datos de verdad y
     las cuatro dan el mismo primer resultado y la misma cantidad que la
     versión corta. Si se cambia alguna, hay que volver a medirlo —no
     alcanza con leerlo— abriendo el chat y corriendo
     `Avisanos.buscar('la frase nueva')` en la consola. */
  function sugerencias(){
    return ['¿Cómo me anoto a las materias?',
            '¿Cómo me anoto para rendir un final?',
            '¿Cómo saco el boleto universitario?',
            '¿Qué becas hay?'];
  }
  ...
  function montar(){
    ...
    /* Si ya la eligió en la app —el inicio, el menú o «Mi año»—, el
       chat no se la vuelve a preguntar. La llave propia del chat queda
       para quien solo la dijo acá, antes de que existiera la otra. */
    const elegida = typeof carreraElegidaApp === 'function' ? carreraElegidaApp() : null;
    try { carrera = (elegida && elegida.id) || localStorage.getItem(LLAVE_CARRERA) || null; } catch(e){}

    const d = decir('bot', `Hola. Te busco entre los trámites y las preguntas
      frecuentes que tenemos cargados. Si no lo tengo, o si preferís
      hablar con una persona, te paso con un compa de la Bolívar.
      <div class="chat-opciones">${sugerencias().map(s =>
        `<button class="chat-opcion pregunta" data-sug="${esc(s)}">${esc(s)}</button>`).join('')}
        <button class="chat-opcion persona" data-persona="1">Te pasamos con un compa de la Bolívar</button></div>`);
    d.querySelectorAll('[data-sug]').forEach(b => {
      b.onclick = () => { d.querySelector('.chat-opciones').remove(); responder(b.dataset.sug); };
    });
    /* Pedido de los estudiantes (18/9/2026): el camino a una persona
       tiene que estar a la vista desde el saludo, no solo cuando el
       buscador no encuentra nada. */
    d.querySelector('[data-persona]').onclick = () => {
      d.querySelector('.chat-opciones').remove();
      decir('yo', 'Quiero hablar con un compa de la Bolívar');
      pasarAPersona();
    };
